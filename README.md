...
- **比べる材料が1つも無いサービス**（対応地域・案件数・手数料率・登録者数・リモート比率が**すべて「非公開」**）も
  検索対象外（2026-09-22 に追加）
  - このサイト自身が AdSense で「有用性の低いコンテンツ」と判定されたための対策。比較サイトなのに、
    検索対象142件のうち41件（29%）は5項目すべてが「非公開（お問い合わせで確認）」で、読んでも比べようがなかった
  - この対策で検索対象は 142件 → 101件になりました
- **カテゴリーページは、検索対象のサービスが1件でも含まれるときだけ検索対象**

静的ページを作り直すかどうかの判定（`prerender.js` の `computeTemplateHash`）には、`index.html` だけでなく
**`lib/indexing.js` も含めています**。線引きを変えても index.html が同じだと、既に書き出したページに
noindex が入らないまま残るためです（2026-09-22 に実際に起きました）。
